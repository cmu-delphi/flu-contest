...
from forecast_io import Targets, Forecast
...
class Forecaster(abc.ABC):
  ...
  def __init__(self, name, test_season, min_week_prob=0.002, min_wili_prob=0.002, forecast_weeks=Utils.get_week_forecast, forecast_wili=Utils.get_wili_forecast, smooth_weeks_bw=1, smooth_wili_bw=1):
    """
  ...
  def forecast(self, epiweek):
    ...
    num_week_bins = forecast.season_weeks
    num_wili_bins = forecast.ili_bins
    ...
    for region in Forecast.regions:
      ...
      # regional info
      baseline = forecast.baselines[region]
      # get all targets
      ...
      forecast.set_onset(region, *onset)
      forecast.set_peakweek(region, *peakweek)
      forecast.set_peak(region, *peak)
      forecast.set_lookahead(region, 1, *x1)
      forecast.set_lookahead(region, 2, *x2)
      forecast.set_lookahead(region, 3, *x3)
      forecast.set_lookahead(region, 4, *x4)
    # sanity check completed forecast
    return forecast.sanity_check(epiweek)
  ...
  def _train(self, region): pass

  @abc.abstractmethod
  def _forecast(self, region, epiweek): pass
