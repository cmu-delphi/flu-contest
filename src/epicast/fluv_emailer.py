...
def already_submitted(cur, user):
  sql = "SELECT count(1) FROM ec_fluv_users u JOIN ec_fluv_submissions s ON s.`user_id` = u.`id` WHERE u.`hash` LIKE '%s%%' AND s.`epiweek_now` = (SELECT max(epiweek) FROM ec_fluv_history)"%(user[0])
  execute_sql(cur, sql)
  num = 0
  for (num,) in cur:
    pass
